...
class RoutePlanNotifier extends StateNotifier<RoutePlanState> {
  ...
  Future<bool> requestRoutePlan(List<String> selectedRoutes) async {
    ...
    try {
      ...
      if (success) {
        ...
        await fetchRouteAndRequestedRoute(state.dateRange);
      }
  ...
  // fetch requested route and route plan both
  Future<void> fetchRouteAndRequestedRoute(DateTimeRange selectedRange) async {
    state = state.copyWith(isLoading: true, dateRange: selectedRange);
    ...
    final routePlanUri = Uri.parse("${Config.backendUrl}/user/route-plan/get");
    final requestedRouteUri = Uri.parse("${Config.backendUrl}/get-requested-route").replace(
      queryParameters: {
        'startDate': selectedRange.start.toIso8601String(),
        'endDate': selectedRange.end.toIso8601String(),
      },
    ...
    try {
      ...
      final approvedRes = await http.post(
        routePlanUri,
        headers: headers,
        body: jsonEncode({"startDate": startDate, "endDate": endDate}),
      );

      final requestedRes = await http.get(requestedRouteUri, headers: headers);

      final List<Map<String, dynamic>> combinedRoutes = [];

      if (approvedRes.statusCode == 200) {
        final jsonApproved = json.decode(approvedRes.body);
        final approvedData = List<Map<String, dynamic>>.from(jsonApproved['data'] ?? []);
        for (var item in approvedData) {
          item['status'] = 'approved'; // normalize if not present
        }
        combinedRoutes.addAll(approvedData);
      }

      if (requestedRes.statusCode == 200) {
        final jsonRequested = json.decode(requestedRes.body);
        final requestedData = List<Map<String, dynamic>>.from(jsonRequested['data'] ?? []);
        combinedRoutes.addAll(requestedData);
      }
      ...
      print("Error in fetchRouteAndRequestedRoute: $e");
      state = state.copyWith(isLoading: false);
    }
  }
